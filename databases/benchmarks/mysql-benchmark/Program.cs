...
Task<int> StartWorkerTaskAsync(int taskId) => Task.Run(async () =>
{
    ...
    await using var conn = new MySqlConnection($"Server=127.0.0.1;Port=3306;Database=teste;Uid=root;Pwd=123456;ApplicationName={appName};");
...
{
    // MySQL não suporta multiplos comandos por conexão, sendo necessário abrir uma connection em cada thread.
    using var conn = new MySqlConnection($"Server=127.0.0.1;Port=3306;Database=teste;Uid=root;Pwd=admin;ApplicationName={appName};");
    conn.Open();
