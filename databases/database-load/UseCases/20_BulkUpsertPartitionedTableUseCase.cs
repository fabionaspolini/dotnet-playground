...
public static class BulkUpsertPartitionedTableUseCase
{
    private const string CreateTemporaryTableSql =
        """
        create temp table transacao_part_temp
        on commit drop
        as table transacao_part with no data
        """;
    
    private const string UpsertSql =
        """
        insert into transacao_part
        select * from transacao_part_temp
    ...
    public static async Task ExecuteAsync(int count)
    {
        ...
        var insertedItems = TransacaoFactory.Generate(count);
        await InternalBulkInsertUseCase.ExecuteAsync(conn, insertedItems, "transacao_part", 50_000);
        ...
        await InternalBulkInsertUseCase.ExecuteAsync(conn, items, "transacao_part_temp", partialMessage: "inseridos na tabela temporária");
